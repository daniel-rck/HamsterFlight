...
web-base-*Tooling* (Bun, oxlint + oxfmt, reusable CI, Hygiene-Dateien) — und sonst
...
3. **Foundation [`daniel-rck/web-base`](https://github.com/daniel-rck/web-base)**
   — nur für Tooling-Fragen (Bun, oxlint/oxfmt, CI, Pins).
...
- **`src/sim/**` bleibt rein und deterministisch.** Kein DOM, kein `performance`,
  ...
  das: `scripts/check-sim-purity.ts`, die `no-restricted-globals`-Deny-Liste in
  `.oxlintrc.json` (mit Begründung pro Eintrag) und `tsconfig.sim.json`
...
- **`reference/` ist vendored.** Dekompilierter Bytecode und
  Extraktions-Tools — nicht von Hand editieren, in `.oxlintrc.json` und `.prettierignore`
  ausgeschlossen.
...
- **Strengere Lint-Regeln als die Basis**: `no-unused-vars` (deckt auch
  ...
  angleichen. `oxlint.base.json` und `.oxfmtrc.json` gehören web-base
  (`web-base check` vergleicht sie byte-genau): nie editieren, Abweichungen
  nur in `.oxlintrc.json` bzw. `.prettierignore`.
