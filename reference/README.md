...
| `as2/timeline/display-lists.txt` | `tools/build_sounds.py` | The game's 21 sounds as MP3 (no transcoding: every DefineSound here is MP3) into `src/assets/sounds/`, and `src/assets/sounds.generated.ts` with each sound's encoder latency and length. |
| `tools/build_screens.py` | The INSTRUCTIONS board (root frame 6) and its Play Now! button as WebP overlays into `src/assets/screens/`, from ffdec's frame and shape SVG exports. |
| `tools/dump_timeline.py` output: per clip and frame, every placement (depth, character, instance name, matrix), label, removal and `StartSound`. The half of the timeline the scripts do not show. |
...
choice by the repository owner rather than an oversight. The same goes for the
sounds under `src/assets/sounds/`, extracted by `tools/build_sounds.py`:
